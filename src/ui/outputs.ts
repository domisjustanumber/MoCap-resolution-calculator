...
function updateBottleneckBanner(type: BottleneckType, app: AppStateFull): void {
  ...
  > = {
    'lens-limited': {
      ...
      icon: '\u25cf',
      text: `Lens-limited: aperture diffraction (f/${state.aperture.toFixed(1)}, ${state.wavelength} nm) cuts detail before the sensor limit. Open aperture or use a shorter wavelength.`,
    },
    'sensor-limited': {
      color: 'border-orange-800 bg-orange-950/30 text-orange-300',
      icon: '\u25a0',
      text: 'Sensor-limited: the pixel grid caps resolution. Increase capture resolution or use a sensor with smaller pixel pitch.',
    },
    ...
    balanced: {
      ...
      icon: '\u2713',
      text: 'Balanced: no single component dominates the bottleneck.',
    },
  };
